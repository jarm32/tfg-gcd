...
Una vez que cada investigador quedó descrito mediante un perfil temático, el análisis podía avanzar desde la representación individual hacia la búsqueda de patrones compartidos. La cuestión, por tanto, se convirtió en revisar si esos perfiles permitían reconocer grupos con intereses próximos.

Este planteamiento se abordó como una tarea de aprendizaje no supervisado, ya que no se buscaba entrenar un modelo para reproducir una clasificación conocida, sino explorar la estructura interna de los perfiles construidos. 

Aunque el IUMPA cuenta con áreas de trabajo asignadas a los investigadores, estas no se mantuvieron como variable de entrada en el modelo de clustering. En las pruebas realizadas, su inclusión hacía que la agrupación quedara fuertemente condicionada por dicha clasificación previa, reuniendo a los investigadores principalmente según el área a la que pertenecían. Por este motivo, se decidió excluir las áreas del análisis y trabajar únicamente con la información derivada de las revistas y de sus palabras clave. De esta forma, el agrupamiento podía explorar posibles afinidades temáticas complementarias a las áreas institucionales, en lugar de limitarse a reproducirlas.
