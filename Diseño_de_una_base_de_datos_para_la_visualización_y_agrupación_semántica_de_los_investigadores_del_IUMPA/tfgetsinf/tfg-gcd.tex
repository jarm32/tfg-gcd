...
Este planteamiento se abordó como una tarea de aprendizaje no supervisado. No se buscaba entrenar un modelo para reproducir una clasificación conocida, sino explorar la estructura interna de los perfiles construidos a partir de revistas y palabras clave. Aunque el instituto cuenta con áreas de trabajo asignadas a los investigadores, estas no se mantuvieron como variable de entrada en el modelo de clustering. En las pruebas realizadas, su inclusión hacía que la agrupación quedara fuertemente condicionada por dicha clasificación previa, reuniendo a los investigadores principalmente según el área a la que pertenecían. Por este motivo, se decidió excluirlas del análisis y trabajar únicamente con la información derivada de las revistas y del vocabulario temático, permitiendo al agrupamiento explorar posibles afinidades complementarias a las áreas institucionales, en lugar de limitarse a reproducirlas.

El proceso de agrupamiento se desarrolló de forma experimental, comparando distintas configuraciones a partir de las representaciones descritas en el apartado anterior. Esta fase no consistió en aplicar un único modelo, sino en probar varias formas de representar los perfiles, medir la proximidad entre ellos y construir agrupaciones. Para ello se trabajó de manera progresiva, comenzando con representaciones basadas en palabras clave explícitas y avanzando posteriormente hacia representaciones semánticas mediante \textit{word embeddings}.

En una primera fase se utilizó la matriz binaria de presencia y ausencia de palabras clave. Esta representación permitía comparar a los investigadores según los términos asociados a las revistas en las que habían publicado, por lo que constituía una aproximación sencilla e interpretable al problema. Sobre esta matriz se probaron medidas de proximidad adecuadas para datos binarios, como Jaccard y Hamming, con el objetivo de comprobar si la coincidencia directa entre palabras clave era suficiente para formar grupos temáticamente coherentes.

Estas primeras pruebas sirvieron como punto de referencia, pero también mostraron las limitaciones de trabajar únicamente con presencia o ausencia de términos. En particular, dos perfiles conceptualmente próximos podían quedar separados si estaban descritos mediante vocabularios distintos. Por ello, se incorporó una segunda representación basada en una versión ponderada de la matriz inicial.

La matriz ponderada mantenía la misma estructura de investigador y palabra clave, pero los valores dejaban de ser únicamente binarios, ya que cada término incorporaba un peso dependiente de su frecuencia. Esta ponderación, basada en la penalización gaussiana, buscaba dar mayor importancia a los términos de frecuencia intermedia y reducir la influencia de palabras excesivamente comunes o demasiado aisladas. Sobre esta representación se probaron medidas aplicables a vectores numéricos, como la distancia euclídea, Manhattan, coseno y correlación. De este modo, se evaluaba si una representación más equilibrada del vocabulario mejoraba la formación de grupos sin abandonar todavía el enfoque basado en palabras clave explícitas.

No obstante, tanto la matriz binaria como la matriz ponderada seguían dependiendo de coincidencias literales entre términos, lo que daba lugar a resultados poco acertados. Por este motivo, el análisis se trasladó posteriormente a las representaciones semánticas obtenidas mediante \textit{word embeddings}. En este caso, cada investigador quedaba representado mediante un vector continuo que resumía el contenido semántico de sus palabras clave. A partir de estos vectores se probaron nuevas configuraciones de clustering, variando tanto la medida de proximidad utilizada como el número de grupos considerado.

Durante esta fase también se valoraron distintas alternativas para generar los agrupamientos. Una posibilidad contemplada fue utilizar mapas autoorganizados o \textit{Self-Organizing Maps} (SOM), que combinan reducción de dimensionalidad y agrupamiento visual de perfiles similares. Esta opción resultaba interesante como vía exploratoria, pero finalmente el análisis se centró en el clustering jerárquico, al ofrecer una estructura más directa para comparar distancias entre investigadores y estudiar distintas particiones del mismo árbol de agrupamiento.

El método de agrupamiento utilizado fue, por tanto, principalmente el clustering jerárquico. Este enfoque resultaba adecuado para el carácter exploratorio del análisis, ya que permite construir una estructura progresiva de agrupación y estudiar distintas particiones del árbol resultante. De esta forma, no era necesario fijar un único número de grupos desde el inicio, sino que podían compararse distintos cortes del dendrograma y valorar qué particiones ofrecían una separación más clara y una interpretación temática más coherente.

Además de dicho método de agrupamiento, también se estudiaron distintas formas de representar los perfiles antes de aplicar el clustering. En particular, se valoraron técnicas de reducción y proyección de dimensionalidad con el objetivo de simplificar la estructura de los embeddings y facilitar la detección de patrones. Entre las alternativas consideradas se encontraban métodos lineales como PCA y enfoques no lineales orientados a preservar relaciones de proximidad entre observaciones.

Dentro de estas alternativas, la técnica que terminó adquiriendo mayor relevancia fue UMAP~\cite{umap}. Este método permite proyectar datos de alta dimensión en espacios de menor dimensión preservando, en la medida de lo posible, la estructura de vecindades entre observaciones. Su uso permitía comparar agrupamientos obtenidos directamente sobre los embeddings con otros generados tras una representación más compacta de los perfiles, comprobando si esta proyección facilitaba la aparición de estructuras temáticas más claras o si, por el contrario, alteraba en exceso la organización original de los datos.

También se tuvo en cuenta la versión del vocabulario empleada. En una primera fase se trabajó con el vocabulario completo, pero durante la revisión de los resultados se detectó que determinadas palabras clave hacían referencia a localizaciones geográficas u otros elementos contextuales. Estos términos podían introducir similitudes poco relevantes para el objetivo del análisis, ya que relacionaban perfiles por elementos externos al contenido científico de las revistas. Por ello, se repitieron las pruebas principales sobre una versión depurada del vocabulario, eliminando las referencias de localización. Esta comparación permitía estudiar si las agrupaciones respondían realmente a afinidades temáticas o si estaban parcialmente influidas por coincidencias contextuales.

Cada configuración del proceso dependía, por tanto, de varios elementos: la representación de entrada, la versión del vocabulario, la medida de proximidad, el método de agrupamiento, el número de grupos considerado y la posible aplicación de reducción de dimensionalidad. La Tabla~\ref{tab:elementos_clustering} resume los componentes principales considerados durante esta fase experimental.
...
\hline
Método de agrupamiento & Clustering jerárquico & Construir grupos de investigadores a partir de la proximidad entre perfiles \\
\hline
Número de grupos & Distintas particiones del árbol jerárquico & Comparar cortes alternativos del dendrograma y valorar su coherencia \\
\hline
Reducción de dimensionalidad & Configuraciones sin reducción y configuraciones con UMAP & Comparar los agrupamientos obtenidos en el espacio original y en una proyección de menor dimensión \\
\hline
Evaluación & Índice de Silhouette, revisión temática y comparación de solapamientos & Comparar configuraciones y valorar la coherencia de las agrupaciones \\
\hline
\end{tabular}
...
La elección de la medida de proximidad dependía del tipo de representación utilizada. En las matrices binarias, medidas como Jaccard o Hamming permitían comparar perfiles según la coincidencia o diferencia en la presencia de palabras clave. En las representaciones ponderadas y en los embeddings, en cambio, resultaban más adecuadas medidas aplicadas sobre vectores numéricos, como el coseno, la correlación, la distancia euclídea o la distancia Manhattan. Esta variedad de medidas permitía comprobar si la estructura de los grupos dependía de una única definición de cercanía o si aparecían patrones similares bajo distintas formas de medir la proximidad temática.
...
Además de comparar el rendimiento individual de cada configuración, se revisó la estabilidad de las particiones obtenidas. Para ello, se analizaron solapamientos entre modelos seleccionados, comparando tanto los investigadores asignados a cada grupo como las palabras clave características de los clusters. Esta revisión permitía valorar si distintos modelos estaban capturando una estructura común en los datos o si, por el contrario, producían segmentaciones muy dependientes de la métrica, de la reducción dimensional o del número de grupos elegido.

Dentro de esta comparación adquirieron especial importancia las configuraciones jerárquicas basadas en distancia de correlación, distancia coseno y distancia coseno tras aplicar UMAP. Estas configuraciones permitían estudiar si los agrupamientos obtenidos eran relativamente estables o si la estructura temática cambiaba de forma notable al modificar la medida de proximidad o al proyectar los datos a un espacio de menor dimensión. De este modo, la evaluación combinó criterios cuantitativos, revisión temática y análisis de estabilidad entre modelos.

El proceso experimental mostró que la representación de entrada tenía un papel determinante en la calidad de los agrupamientos. Las configuraciones basadas en coincidencias literales permitieron establecer una primera referencia, pero también evidenciaron las limitaciones de trabajar únicamente con presencia o ponderación de palabras clave. Por ello, las configuraciones basadas en embeddings, especialmente al compararlas con y sin reducción de dimensionalidad, adquirieron un papel central en la búsqueda de agrupaciones más coherentes e interpretables.
