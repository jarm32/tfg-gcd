...
freq <- rowSums(X)
mu <- mean(freq)
sigma <- sd(freq) * 0.5 # Suaviza la penalización

# Penalización gaussiana
...
X_for_dist <- X_for_dist[apply(X_for_dist, 1, var) > 0, , drop = FALSE]
X_for_dist <- X_for_dist[, apply(X_for_dist, 2, var) > 0, drop = FALSE]
...
# Visualiza la curva de ponderación
