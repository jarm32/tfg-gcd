...
#################
# Ponderación gaussiana por frecuencia de palabra
#################

freq <- rowSums(X)
mu <- mean(freq)
sigma <- sd(freq) * 1 # 0'75 penaliza más los extremos y 2 menos

# Calcula el peso gaussiano --> penalización simétrica
weights <- exp(-((freq - mu)^2) / (2 * sigma^2))

# Normaliza para que los pesos estén entre 0 y 1
weights <- (weights - min(weights)) / (max(weights) - min(weights))

X_weighted <- X * weights
X <- X_weighted

message("Distribución de pesos:")
...
# Visualizar la curva de ponderación para verificar que no aplana demasiado los extremos
plot(freq, weights, pch=19, col="steelblue",
     main="Ponderación gaussiana de keywords",
     xlab="Frecuencia", ylab="Peso aplicado")
