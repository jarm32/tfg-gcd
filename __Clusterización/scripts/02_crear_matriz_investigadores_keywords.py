...
df_revistas = pd.read_excel("../data/One_Hot_Palabras_Revistas_filtrado.xlsx")
df_revistas.set_index(df_revistas.columns[0], inplace=True) # Se accede con df_revistas.index

path = "." 
files = glob.glob("../data/publicaciones_investigadores/1ha_publicado_en_*_editoriales.csv")
...
df_investigadores.to_excel("../data/Matriz_Investigadores_Keywords.xlsx")

df_relacional = pd.DataFrame(relacional, columns=["Investigador", "Revista", "Keyword"])
df_relacional.to_excel("../data/Trazabilidad_Keywords_Revistas.xlsx", index=False)
