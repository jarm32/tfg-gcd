...
M <- as.data.frame(readxl::read_excel("../data/Matriz_Investigadores_Keywords_sinlocl.xlsx"))
rownames(M) <- M[[1]]
M <- M[,-1]
M[is.na(M)] <- 0

E <- read.csv("../data/Embeddings_keywords_sinlocl.csv", row.names = 1)
E <- as.matrix(E)
...
write.csv(investigadores_vec, "../data/Matriz_Investigadores_Embeddings_sinlocl.csv", row.names = TRUE)
