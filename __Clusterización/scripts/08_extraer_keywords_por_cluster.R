...
ruta_excel <- "../data/Matriz_Investigadores_Embeddings_sinlocl.csv"
...
emb <- read.csv("../data/Matriz_Investigadores_Embeddings_sinlocl.csv", row.names = 1)
...
dir.create("../results/UMAP_Resultados", showWarnings = FALSE, recursive = TRUE)

# 1. Guardar la tabla final completa (investigadores + cluster + UMAP + silhouette)
write.csv(
  tabla_final,
  "../results/UMAP_Resultados/UMAP_Clusters_Investigadores.csv",
  row.names = FALSE
)
...
write.csv(
  umap_df,
  "../results/UMAP_Resultados/UMAP_Coordenadas.csv",
  row.names = TRUE
)
...
saveRDS(
  umap_config_list,
  file = "../results/UMAP_Resultados/UMAP_Parametrizacion.rds"
)

# 4. Guardar el objeto del modelo UMAP completo
saveRDS(
  umap_res,
  file = "../results/UMAP_Resultados/UMAP_Modelo.rds"
)
...
mat_keywords <- read_excel("../data/Matriz_Investigadores_Keywords_sinlocl.xlsx")
...
write.csv(
  ...
  "../results/UMAP_Resultados/Keywords_representativas_por_cluster.csv",
  row.names = FALSE
)
